...
function ect_sitemap_fun()
{
	...
	echo '<h2>Exclude Posts&nbsp;<small>(Mark to exclude posts.)</small></h2>';
			if(is_array($DataArr['post']) && !empty($DataArr))
			{
				echo '<ul class="admin_inner">';
				asort($DataArr['post']);
...
/*	echo do_shortcode('[ECT_HTML_SITEMAP]');*/
	...
	if(!empty($_POST))
	{
		$ect_sitemap_is_seo=esc_sql($_POST['ect_sitemap_is_seo']);
		$exclude=esc_sql($_POST['exclude']);
		$excludepost=esc_sql($_POST['excludepost']);
		$excludeprod=esc_sql($_POST['excludeprod']);
		
		$IsSeo=isset($ect_sitemap_is_seo) ? 1 : 0;
		update_option('ect_sitemap_excluded_pages',$exclude);
		update_option('ect_sitemap_excluded_posts',$excludepost);
		update_option('ect_sitemap_excluded_prods',$excludeprod);
...
function MapUpdater()
{
	$ect_sitemap_update=$_GET['ect_sitemap_update'];
	$LastUpdate=get_option('ect_sitemap_last_update',true);
	$NextUpdate=mktime(date('H',$LastUpdate),date('i',$LastUpdate),date('s',$LastUpdate),date('m',$LastUpdate),date('d',$LastUpdate)+1,date('Y',$LastUpdate));
	if(time()>$NextUpdate || isset($ect_sitemap_update))
	{	
		$XmlFile='sitemap.xml';
		GenerateXmlSiteMap($XmlFile);
		update_option('ect_sitemap_last_update',time());
		if(isset($ect_sitemap_update))
...
function GenerateXmlSiteMap($XmlFile)
{
	...
	fwrite($fp,$Str);
	fclose($fp);
	chmod('../'.$XmlFile, 0777);  //changed to add the zero
}
...
function ListStoreProd1()
{
	...
	{
		//and pInStock>0
		$ProdArr=$ECTWPDB->get_results("select pID,pName from products where pDisplay=1  order by pName");
		if(!empty($ProdArr))
			return $ProdArr;
	}
}
...
if(is_array($ExcludedPages) && is_array($ExcludedPosts))
	{
		$ExtIDs=array_merge($ExcludedPages,$ExcludedPosts);	
	}
	elseif(!empty($ExcludedPages) && empty($ExcludedPosts))
	{
		$ExtIDs=$ExcludedPages;
	}
	elseif(empty($ExcludedPages) && !empty($ExcludedPosts))
	{
		$ExtIDs=$ExcludedPosts;
	}
	
	$ExQ='';
	if(empty($NoC) && !empty($ExtIDs) && is_array($ExtIDs))
	{
		$ExQ='and ID NOT IN ('.implode(',',$ExtIDs).')';
	}	
...
{ 
	...
	return $Data=array('Categories'=>array('Url'=>site_url('/').'categories.php','PostTitle'=>'Categories','LastModify'=>$LastM));
}
